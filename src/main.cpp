...
#include "Eigen/Eigen"
#include "algorithms/sensorfusion.h"
...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0; i < argc; i++) {
        input_file_name.push_back(*argv[i]);
    }
    */
    ...
    while (getline(input_file, line)) {
	    ...
	    gt_package.gt_values_ = Eigen::VectorXd(3);
        gt_package.gt_values_ << x_gt, y_gt, v_gt;
    ...
    std::cout << measurement_pack_list.size() << std::endl;
    for (size_t i = 0; i < measurement_pack_list.size(); ++i) {
    	//std::cout << "Fusion Time: " << i <<  std::endl;    
        fuser.Process(measurement_pack_list[i]);
    	//std::cout << "Process done" << std::endl;    
        Eigen::VectorXd x_out = fuser.kf_.GetX();
        // 输出跟踪后的位置
        std::cout << x_out(0) << "     "
                  << x_out(1) << "     "
                  << x_out(2) << "     "
